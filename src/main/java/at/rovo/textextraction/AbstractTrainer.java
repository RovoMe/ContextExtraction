...
/**
 * <p>
 * This abstract base class provides some common methods for all training
 * strategies like loading and persisting of common tags.
 * </p>
 */
public abstract class AbstractTrainer implements TrainingDataStrategy
{
	/** The logger of this class **/
	private static Logger LOG = LogManager.getLogger(AbstractTrainer.class);
	/** The directory which contains the training data **/
	protected final File trainingDir;
	/** The number of samples to train the classifier for each source. By
	 * default 2000 samples will be trained **/
	...
	/**
	 * <p>
	 * Determines if in a previous iteration common tags were persisted. These
	 * method loads data from <code>commonTags.ser</code> into memory and
	 * provides access to the loaded data via the <em>commonTags</em> field
	 * of this class.
	 * </p>
	 *
	 * @return true if there was already a <em>commonTags.ser</em> file within
	 *         the directory that contains the training data, false otherwise
	 */
	protected boolean isCommonTagsAvailable()
	{
		...
		FilenameFilter filter = new FilenameFilter()
		{
			...
			{
				return name.equals("commonTags.ser");
			}
		};
	 ...
	 *
	 * @param serializedObject A reference to the {@link java.io.File} representing
	...
	protected Dictionary<String, List<String>> loadCommonTags(File serializedObject)
	{
		...
		try (ObjectInputStream ois = new ObjectInputStream(new BufferedInputStream(new FileInputStream(serializedObject))))
		{
	...
	protected Classifier<String, String> loadExistingData(Classifier<String, String> cls)
	{
		...
		FilenameFilter filter = new FilenameFilter()
		{
			public boolean accept(File dir, String name)
			{
				return name.endsWith("_"+trainingSampleSize+"_"+ trainFeatureStrategy.name()+".ser");
			}
		};
		// check the training directory if there are any serialized objects
		// from previous trainings
		...
		if (serObjects.length > 0)
		{
			...
			{
				LOG.warn("On trying to load already trained data multiple files were found. Using the first one found.");
			}
