...
/**
 * <p>This class is the base class for every maximum subsequence 
 * segmentation (MSS) algorithm.</p>
 * <p>It provides the actual linear time cost implementation of the MSS 
 * algorithm as presented by Jeff Pasternack and Dan Roth in their paper 
 * 'Extracting Article Text from the Web with Maximum Subsequence 
 * Segmentation' as well as the original algorithm presented by Walter Ruzzo 
 * and Martin Tompa in their paper 'A linear time algorithm for finding 
 * all maximal scoring subsequences'.</p>
 * <p>Extending classes have to implement {@link #predictText(List)}</p>
 * 
 * @author Roman Vottner
 */
public abstract class MaximumSubsequenceSegmentation extends TextExtractor
{
	private static Logger logger = LogManager.getLogger(MaximumSubsequenceSegmentation.class.getName());
	
	/**
	 * <p>Provides basic initialization for extending classes</p>
	 */
	...
	/**
	 * Represents the algorithm presented by Jeff Pasternack and Dan Roth
	 * which finds the subsequence with the highest score in a provided
	 * {@link List} within linear time.
	 * 
	 * @param s A {@link List} of {@link Double} values representing a score 
	 *          out of which a subsequence should be extracted that produces
	 *          a highest score
	 * @param maxSS An empty {@link List} should be provided, this {@link List}
	 *              will contain the maximum subsequence after the call
	 * @return The starting index of the subsequence based on it's position
	 *         in s 
	 */
...
//		logger.debug("Maximum subsequence: "+maxSS);
	...
	/**
	 * <p>Calls the original version of the algorithm and returns only the 
	 * most interesting parameters.</p>
	 * <p>The {@link List} with all subsequences is returned via parameter
	 * reference while the start positions of every subsequence relating to 
	 * their position in the origin list is returned as an array via the
	 * methods return method.</p>
	 * 
	 * @param S A {@link List} of {@link Double} values representing a score 
	 *          out of which a subsequence should be extracted that produces
	 *          a highest score
	 * @param kSS An empty {@link List} should be provided, this {@link List}
	...
	public int[] kMaximumSubsequenes(final List<Double> S, List<List<Double>> kSS)
	{
		...
		this.kMaximumSubsequences(S, kSS, new ArrayList<Double>(), new ArrayList<Double>(), startPos);
	...
	/**
	 * <p>Implements the maximal scoring subsequences algorithm as proposed by
	 * Walter Ruzzo and Martin Tompa in their paper 'A linear time algorithm 
	 * for finding all maximal scoring subsequences'</p>
	 * <p>This methods takes a parameter S which contains the original sequence
	 * and returns a {@link List} via object-reference (Parameter I) with all 
	 * subsequences which are further organized in a {@link List}. Moreover 
	 * the score from sequence-start up to but not including the position of
	 * the subsequence is returned via object reference in parameter L and the
	 * score including the subsequence score is returned via parameter R. 
	 * Furthermore the start positions in the original sequence of every 
	 * subsequence found is returned via the startPos parameter.</p>
	 * <p>Note that the content of the last three parameters does get cleared
	 * if there is one.</p>
	 * 
	 * @param S The sequence where subsequences should get extracted
	 * @param I Certain ordered list of disjoint subsequences
	 * @param L cumulative total of Ii of all scores up to but not 
	 *          including the leftmost score of Ij
	 * @param R List of the total score up the the specific element
	 * @param startPos Returns the start position of every subsequence found
	 *                 in the original list
	 * @return The total score of the sequence
	 */
	public double kMaximumSubsequences(final List<Double> S, List<List<Double>> I, List<Double> L, List<Double> R, List<Integer> startPos)
	{
		if (S == null)
		 ...
		 * 4. Otherwise (i,e. there is such a j, but Rj < Rk), extend the subsequence Ik to the left
		 *    to encompass everything up to and including the leftmost score in Ij. Delete subsequences
		 *    Ij, Ij+1, ... Ik-1 from the list (none of them is maximal) and reconsider the newly extended 
		 *    subsequence Ik (now renumbered Ij) as in step 1.
		 */
		double cumulativeTotal = 0.;
		// The list is initially empty
		I.clear(); L.clear(); R.clear(); startPos.clear();
		for (int i=0; i<S.size(); i++)
		{
	...
	/**
	 * <p>Implements the 4 steps introduced by Walter Ruzzo and Martin Tompa. This
	 * is necessary as the 4th step calls this method again without reading the next
	 * part of the input sequence.</p>
	 * 
	 * @param i Current position in the origin sequence
	 * @param cumulativeTotal Sum of all elements from the start to but not including 
	 *                        the i'th element 
	 * @param S The origin sequence
	 * @param I A {@link List} of all found subsequences up to the i'th element
	 * @param L The cumulative values before the corresponding subsequence
	 * @param R The cumulative values after the corresponding subsequence
	 * @param startPos The start position in the origin sequence of a subsequence 
	 * @param append Decides if this method should consider the next value of the
	 *               origin sequence (append == false) or use the latest subsequence
	 *               for further processing
	 */
	private void applySteps(int i, double cumulativeTotal, List<Double> S, List<List<Double>> I, List<Double> L, List<Double> R, List<Integer> startPos, boolean append)
	{
		...
		// 4. Otherwise (i.e., there is such a j, but Rj < Rk), extend the subsequence Ik
		// to the left to encompass everything up to and including the leftmost score in Ij.
		// Delete subsequences Ij, Ij+1, ..., Ik-1 from the list (none of them is maximal)
		// and reconsider the newly extended subsequence Ik (new renumbered Ij) as in step 1.
		else 
		{
			// join Ij to Ik
	 ...
	 * 
	 * @param seq The sequence whose value should get calculated
	...
	/**
	 * <p>Predicts the main content of the HTML page</p>
	 * 
	 * @param url The URL of the page to predict content from
	 * @return A {@link List} of the predicted main content
	 * @throws Throws an {@link ExtractionException} if an error occurs
	 *         during the text prediction
	 */
	public abstract String predictText(final String url) throws ExtractionException;
	
	/**
	 * <p>Predicts the main content of the provided HTML pages</p>
	 * 
	 * @param urls The URLs of the pages to predict content from
	 * @return A {@link List} of the predicted contents for the provided
	 *         pages
	 * @throws Throws an {@link ExtractionException} if an error occurs
	 *         during the text prediction
	 */
	...
	/**
	 * <p>Cleans the extracted blocks as proposed by Jeff Pasternack and
	 * Dan Roth in their paper on 'Extracting article text from the web 
	 * with maximum subsequence segmentation'.</p>
	 * 
	 * @param text The extracted text from a web page via the 
	 *             {@link #topMaximumSubsequence(List, List)} method
	 * @param renameUncommonTags Specifies if tags which aren't included in
