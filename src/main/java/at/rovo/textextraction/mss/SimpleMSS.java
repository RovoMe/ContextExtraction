...
/**
 * <p>This class represents the simplified maximum subsequence segmentation approach
 * presented by Jeff Pasternack and Dan Roth in their paper on 'Extracting Article 
 * Text from the Web with Maximum Subsequence Segmentation'</p>
 * <p>The simple method predicts article text through assigning a static score of 
 * -3.25 to every tag and +1 to every word upon building a score-list which will
 * be used by the maximum subsequence segmentation algorithm to find the subsequence
 * with the highest value.</p>
 * <p><b>Note:</b> The simple maximum subsequence segmentation does not rely on any
 * learning or training mechanism!</p>
 * 
...
public class SimpleMSS extends MaximumSubsequenceSegmentation
{
	...
	/**
	 * <p>Predicts article text based on assigning a score of -3.25 to every tag and
	 * +1 to every word and building a score-list using those scores for every 
	 * token in the provided parameter.</p>
	 * <p>This score-list is now used to run the maximum subsequence segmentation 
	 * algorithm and to find the subsequence with the highest value.</p>
	 * <p>This start position of the sequence and the length of the returned array
	 * are now used to find the article text in the provided list of tokens</p>
	 * 
	 * @param url The URL of the page to predict its content from
	 * @return The predicted article of the provided URL; null if the HTML page could 
	 *         not be read
	 * @throws NoSubsequenceFoundException If no subsequence could be calculated
	 */
	@Override
	 ...
	 * @return The predicted articles of the provided URLs; null if the HTML page could 
	 *         not be read
	 * @throws NoSubsequenceFoundException If no subsequence could be calculated
	 */
	@Override
