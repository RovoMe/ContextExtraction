...
/**
 * Zhang and Lin have presented a novel approach on content extraction through using two similar news pages as input set
 * which are passed to the template generation algorithm <em>{@link #templateGeneration}</em>.
 * <p>
 * Based on two similar pages <em>{@link #improvedSimpleTreeMatching}</em> builds a maximum matching tree <em>tm</em>,
 * which is done by comparing nodes within the DOM-trees of both of the similar pages. The maximum matching tree is used
 * by <em>{@link #treeExtraction}</em> to build a template which is than used by <em>{@link
 * #newsContentExtraction(Deque, Token)}</em> to extract the article's main body.
 *
...
public class TemplateExtraction
{
    ...
    private LinkedList<Token> matchedChildren = new LinkedList<>();
    ...
    /** Extraction template * */
    private Deque<Token> tt = new LinkedList<>();

    public static void main(String[] args)
    {
//        String url1 = "<html><head><title>Testpage 1</title></head><body><p>This is an example text.</p></html>";
//        String url2 =
//                "<html><head><title>Testpage 2</title></head><body><p>This is a further example text.</p><p>It even contains a sentence more.</p></html>";
        ...
        LOG.info("Content page 1:");
        Deque<Token> template = te.templateGeneration(url1, url2);
        te.newsContentExtraction(template, te.buildDOMTree(url1).get(0));
        LOG.info("");
        LOG.info("Content page 2:");
        template = te.templateGeneration(url2, url1);
        te.newsContentExtraction(template, te.buildDOMTree(url2).get(0));
    }

    /**
     * Takes the total content of two HTML pages as full String as input parameter and compares these two pages on
     * node-level of their corresponding DOM-trees to extract a common template for the first input page.
     *
     * @param url1
     *         The first page
     * @param url2
     *         The second page which should be very similar to the first page (e.g. taken from the same source)
     *
     * @return The extraction template <em>tt</em>, which can be used by the news content extraction ({@link
     * #newsContentExtraction(Deque, Token) NCE}) algorithm
     */
    public Deque<Token> templateGeneration(String url1, String url2)
    {
        this.ta = this.buildDOMTree(url1);
        LOG.info("URL1 contains " + ta.size() + " nodes");
        this.tb = this.buildDOMTree(url2);
        LOG.info("URL2 contains " + tb.size() + " nodes");
        this.tm = new ArrayList<>(this.ta.size());

        // Simple tree matching and backtracking (STMB) algorithm based on simple tree matching (STM) and longest common
        // subsequence (LCS)

        // improved simple tree matching algorithm
        this.improvedSimpleTreeMatching(0, 0);
        LOG.info("comparedNodes and matrices calculated");
        // simple tree matching and backtracking algorithm
        this.tm = this.maximumMatchingTreeBacktracking(0);
        LOG.info("calculating backtracking");

        // template extraction algorithm
        this.treeExtraction(0);
        LOG.info("generated extraction template");
        return this.tt;
    }

    /**
     * Initializes the <em>matchedMatrix</em>-, <em>comparedNode</em>- and <em>comparedMatrix</em>-fields of nodes in
     * the DOM-tree of ta.
     * <p>
     * It first compares a node taken from both trees for equal names and if there names do not match their subtree do
     * not match either. But if they match the algorithm recursively finds the maximum matching between the subtrees
     * rooted by the children of ta[p] and tb[q].
     * <p>
     * Therefore it initializes two matrices: m and f. The primer is the maximum matched nodes matrix while the latter
     * is the maximum matched path matrix. <p/> As f is not always the true <em>matchedMatrix</em> because ta[p] may
     * match multiple nodes in tb, it is only treated as the initial value of matchedMatrix.
     * <p>
     * ta[p]'s <em>comparedNode</em> set will store all tb's nodes which have been compared with ta[p], while ta[p]'s
     * <em>comparedMatrix</em> stores the related matching path flag matrix <em>f</em> in order to find the true matched
     * Matrix of ta[p] in the sequent algorithms.
     *
     * @param p
    ...
    private int improvedSimpleTreeMatching(int p, int q)
    {
        ...
        // compare names, if they are distinct, the subtree rooted by them do not match at all
        if (!this.ta.get(p).getName().equals(this.tb.get(q).getName()))
        {
            this.ta.get(p).addComparedMatrices(null);
            return 0;
        }
        // if they match however, the algorithm recursively finds the maximum matching between the subtrees rooted by
        // the children ta[p] and tb[q]
        else
        {
    ...
    /**
     * As the improved simple matching tree ({@link #improvedSimpleTreeMatching(int, int) ISTM}) algorithm does not
     * provide any backtracking information, this algorithm uses the matched children backtracking algorithm ({@link
     * #matchedChildrenBacktracking(int, int[][], int, int) MCB}) to get its matched children.
     *
     * @param p
     *         The index of the p-th element of the DOM-tree ta
     */
    private List<Token> maximumMatchingTreeBacktracking(int p)
    {
        ...
        this.tm.get(p).setChildren(new LinkedList<>());
    ...
    /**
     * Matched children backtracking algorithm</p> <p>Besides finding the matched children using ta's
     * <em>matchedMatrix</em> field, the main job is to modify the initial <em>matchedMatrix</em> of each matched
     * child.
     *
     * @param p
     *         The index of the p-th element of the DOM-tree ta
     * @param matchedMatrix
     *         The matched tree path matrix of ta for node p
     * @param i
     *         The x-position in the matchedMatrix to look for the value
     * @param j
     *         The y-position in the matchedMatrix to look for the value
     */
    private void matchedChildrenBacktracking(int p, int[][] matchedMatrix, int i, int j)
    {
        // slight modification as if the 1st page is larger than the second or a segment of the 1st page contains more
        // children than the 2nd page, j might become negative while i is 0. To my understanding if either of i or j
        // gets 0 the border of the matrix was reached.
        if (i == 0 && j == 0)
        {
            return;
        }
    ...
    /**
     * Generates an extraction template <em>tt</em>, which is the set of non-noisy nodes by depth-first traversal of the
     * maximum matching tree <em>tm</em> which only contains nodes that matched in both ta and tb trees.
     * <p>
     * If two pages have the same sub-trees, these fractions are considered noise. As anchor-tags are hidden noise text,
     * the anchor-text-ratio of a sub-segment has to exceed a given threshold to be excluded.
     * <p>
     * Moreover punctuation metrics are used to further separate noise from content.
     *
     * @param i
    ...
    private Deque<Token> treeExtraction(int i)
    {
            ...
            !(this.tm.get(i).getPunctNum() == 0 && this.tm.get(i).getSegNum() >= 3))
        {
            ...
            // expand extraction range of tt to extract content that did not matched in the maximum matching tree tm as
            // they occurred in tb but not in ta
    ...
    /**
     * Uses the extraction template generated by the {@link #templateGeneration} process and utilizes some own
     * characteristics of the target page pt to extract the content of it which first needs to be parsed to a DOM tree
     * <em>tp</em>.
     *
     * @param tt
     *         The previouslz generated extraction template
     * @param tp_i
     *         The root DOM element of the document to predict the main content for
     */
    ...
    /**
     * Removes embedded noise from the provided nodes subtree text.
     *
     * @param subtreeText
     *         The content of the nodes subtree
     *
     * @return The content of the provided subtree node cleaned from embedded noise
     */
    private String deleteEmbeddedNoise(String subtreeText)
    {
        return subtreeText;
    }

    /**
     * Generates a document object model (DOM) like tree of the tokens found within the provided <em>url</em>.
     *
     * @param url
     *         If the <em>url</em> starts with <em>http://</em> the content of the URL is downloaded and parsed else the
     *         <em>url</em> is considered to be a {@link String} containing the content of the page to generate a DOM
     *         like tree for
     *
     * @return A list of tokens which have DOM like properties set
     */
    public List<Token> buildDOMTree(String url)
    {
        ...
        // remove invalid HTML tags including:
        // <comment>, <style>, <script>, <noscript>, <img>, <form>, <input> and <select>
